        ...
        {SPHERES_META.map((meta, idx) => {
          ...
          return (
            <motion.div
              ...
              className="p-4 rounded-2xl bg-white/[0.02] border border-white/[0.04] flex flex-col gap-2 relative overflow-hidden"
            >
              ...
              <p className="text-xs text-white/60 leading-relaxed font-light">
                {sphereData?.state_description || "Глава истории в этой сфере еще не синтезирована в Океан. Проходите сессии синхронизации для углубления этого знания."}
              </p>
              
              {sphereData?.key_lesson && (
                <div className="mt-2 text-[10px] text-violet-400/80 italic border-l border-violet-500/30 pl-3 py-0.5">
                  Вызов: {sphereData.key_lesson}
                </div>
              )}
            </motion.div>
          );
