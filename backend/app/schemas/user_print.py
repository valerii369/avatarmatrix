...
class Identity(BaseModel):
    ...
    summary: str = Field(..., description="Глубинное описание сути")
    core_archetype: str = Field(..., description="Доминирующий архетип")
    narrative_role: str = Field(..., description="Роль в текущем жизненном сюжете")
    energy_description: str = Field(..., description="Как человек проявляется энергетически")
    archetypal_resonance: Dict[str, str] = Field(default_factory=dict, description="Отношение к ключевым силам (напр. 'Власть: Страх', 'Любовь: Поиск')")
...
class SphereNarrative(BaseModel):
    ...
    state_description: str = Field(..., description="Нарратив состояния")
    active_conflict: Optional[str] = Field(None)
    central_symbols: List[str] = Field(default_factory=list)
    evolution_stage: str = Field(..., description="Стадия развития")
