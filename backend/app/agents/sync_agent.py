...
def get_response_metrics(text: str) -> dict:
    ...
    body_parts = ["грудь", "живот", "горло", "рук", "ног", "спин", "плеч", "голов", "солнечн", "дыхани", "сердц", "тело", "мышц", "колен", "стоп", "пальц"]
    has_body = any(bp in text_lower for bp in body_parts)
    
    # Space objects / environment keywords (Russian)
    space_objects = ["стена", "пол", "потолок", "дверь", "окно", "туман", "свет", "тьма", "камень", "дерево", "вода", "песок", "стол", "стул", "предмет", "вещь"]
...
{{
  ...
  "mental_thinking": "тип мышления (логика/образы/страх/отрицание)",
  "mental_reactions": "типичные телесные реакции на давление",
  "mental_patterns": "сценарный паттерн поведения",
  "mental_aspirations": "скрытое стремление из Слоя 5",
  "hawkins_score": 100,
