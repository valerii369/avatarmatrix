...
КЛЮЧЕВОЙ ОБРАЗ (СИМВОЛ) ИЗ СИНХРОНИЗАЦИИ: {recurring_symbol if recurring_symbol else "Не определен"}
АКТУАЛЬНЫЙ ЖИЗНЕННЫЙ КОНТЕКСТ:
{memory_context if memory_context else "Нет данных о последних событиях."}
...
4. СИМВОЛИЧЕСКАЯ РАБОТА: Если символ определен, работай именно с ним. Свет архетипа должен наполнять этот образ.
5. КОРОТКО: Ответ 2-4 предложения. 
6. ОДИН ВОПРОС: Всегда заканчивай одним открытым вопросом.
7. ФИНАЛ: В 3-м протоколе обязательно доведи до конкретного микро-действия в реальности.
8. МЕТРИКИ ОЦЕНКИ (JSON поля):
